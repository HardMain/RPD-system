...
DEFAULT_LLM_PROMPTS: list[dict] = [
    {
        ...
        "user_prompt_template": (
            ...
            "- Задачи — 5-7 пунктов маркированным списком."
        ),
    },
    {
        "section_key": "objects",
    ...
    },
    {
        ...
        "user_prompt_template": (
            ...
            "2. Иначе — используй пример из согласованной РПД-образца, если он передан.\n"
            "3. Если ни того, ни другого нет — действуй по правилам ниже (в реальных РПД именно так).\n\n"
            "Правила распределения часов (соблюдать всегда, даже при наличии примера):\n"
    ...
    },
    {
        ...
        "user_prompt_template": (
            ...
            "3. Если ни того, ни другого нет — действуй по правилам ниже (в реальных РПД именно так).\n\n"
            "Правила (при отсутствии примера):\n"
            "- Опиши используемые технологии и методы обучения по видам занятий (лекции, практические, "
            "лабораторные, СРС) с учётом распределения часов; укажи формы активного и интерактивного "
            "обучения, если применимы."
        ),
    },
    {
        ...
        "user_prompt_template": (
            ...
            "3. Если ни того, ни другого нет — действуй по правилам ниже (в реальных РПД именно так).\n\n"
            "Правила (при отсутствии примера):\n"
            "- Рекомендации по подготовке к лекциям, практическим и лабораторным занятиям, "
            "к самостоятельной работе и к промежуточной аттестации."
        ),
    },
    {
        ...
        "user_prompt_template": (
            ...
            "Дисциплина: {discipline}\nНаправление: {direction}\n\n"
            "Приоритет источников:\n"
            "1. Если в дополнительных материалах есть раздел «Основная литература» (загруженный документ) — "
            "ориентируйся на тех же издателей и тот же стиль библиографического описания.\n"
            "2. Иначе — используй пример из согласованной РПД-образца, если он передан.\n"
            "3. Если ни того, ни другого нет — действуй по правилам ниже (в реальных РПД именно так).\n\n"
            "Правила (соблюдать всегда):\n"
            "- Если ниже передан справочник литературы по дисциплине — при совпадении по смыслу бери из него "
            "точное библиографическое описание издания.\n"
    ...
    },
    {
        ...
        "user_prompt_template": (
            ...
            "Дисциплина: {discipline}\nНаправление: {direction}\n\n"
            "Приоритет источников:\n"
            "1. Если в дополнительных материалах есть аналогичный подраздел (загруженный документ) — "
            "ориентируйся на тех же издателей и стиль описания.\n"
            "2. Иначе — используй пример из согласованной РПД-образца, если он передан.\n"
            "3. Если ни того, ни другого нет — действуй по правилам ниже (в реальных РПД именно так).\n\n"
            "Правила (соблюдать всегда):\n"
            "- Если ниже передан справочник литературы по дисциплине — при совпадении по смыслу бери из него "
    ...
    },
    {
        ...
        "user_prompt_template": (
            ...
            "Дисциплина: {discipline}\nНаправление: {direction}\n\n"
            "Приоритет источников:\n"
            "1. Если в дополнительных материалах есть подраздел «Периодические издания» (загруженный документ) — "
            "ориентируйся на тот же стиль и формат.\n"
            "2. Иначе — используй пример из согласованной РПД-образца, если он передан.\n"
            "3. Если ни того, ни другого нет — действуй по правилам ниже (в реальных РПД именно так).\n\n"
            "Правила (соблюдать всегда):\n"
            "- Если ниже передан справочник литературы по дисциплине — при совпадении по смыслу бери из него "
    ...
    },
    {
        ...
        "user_prompt_template": (
            ...
            "Приоритет источников:\n"
            "1. Если в дополнительных материалах есть аналогичный подраздел (загруженный документ) — "
            "ориентируйся на стиль и формат.\n"
            "2. Иначе — используй пример из согласованной РПД-образца, если он передан.\n"
            "3. Если ни того, ни другого нет — действуй по правилам ниже (в реальных РПД именно так).\n\n"
            "Правила (соблюдать всегда):\n"
            "- Если ниже передан справочник литературы по дисциплине — при совпадении по смыслу бери из него "
            ...
            "- Если по дисциплине нормативных документов нет — верни пустой массив [].\n\n"
            "Формат ответа — строго JSON массив без пояснений до/после:\n"
    ...
    },
    {
        ...
        "user_prompt_template": (
            ...
            "Дисциплина: {discipline}\nНаправление: {direction}\n\n"
            "Приоритет источников:\n"
            "1. Если в дополнительных материалах есть пример этого подраздела (загруженный документ) — "
            "ориентируйся на его стиль.\n"
            "2. Иначе — используй пример из согласованной РПД-образца, если он передан.\n"
            "3. Если ни того, ни другого нет — действуй по правилам ниже (в реальных РПД именно так).\n\n"
            "Правила (соблюдать всегда):\n"
            "- Если ниже передан справочник литературы по дисциплине — при совпадении по смыслу бери из него "
    ...
    },
    {
        ...
        "user_prompt_template": (
            ...
            "Дисциплина: {discipline}\nНаправление: {direction}\n\n"
            "Приоритет источников:\n"
            "1. Если в дополнительных материалах есть пример раздела «ПО» (загруженный документ) — "
            "ориентируйся на тот же набор.\n"
            "2. Иначе — используй пример из согласованной РПД-образца, если он передан.\n"
            "3. Если ни того, ни другого нет — действуй по правилам ниже (в реальных РПД именно так).\n\n"
            "Правила (соблюдать всегда):\n"
            "- Если ниже передан справочник наименований ПО — при совпадении по смыслу бери из него "
            "ТОЧНОЕ полное наименование, не сокращай и не переименовывай.\n"
    ...
    },
    {
        ...
        "user_prompt_template": (
            ...
            "ориентируйся на ту же группировку помещений и формат описания оборудования.\n"
            "2. Иначе — используй пример из согласованной РПД-образца, если он передан.\n"
            "3. Если ни того, ни другого нет — действуй по правилам ниже (в реальных РПД именно так).\n\n"
            "Правила (соблюдать всегда):\n"
            ...
            "- Сгруппируй по типам помещений с учётом видов занятий и часов; для каждого помещения "
            "укажи перечень оборудования и количество.\n\n"
            "Формат ответа — строго JSON массив без пояснений до/после:\n"
